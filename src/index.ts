			...
			.array(
				z.object({
					...
					values: z
						.array(z.string())
						.optional()
						.describe('Values for multi-valued fields like multiselect'),
				}),
			)
			.optional()
			.describe('Custom fields to set on the account'),
	},
	async (params) => {
			...
			.array(
				z.object({
					...
					value: z
						.string()
						.optional()
						.describe('Value for single-valued fields. Unset to remove'),
					values: z
						.array(z.string())
						.optional()
						.describe('Values for multi-valued fields like multiselect'),
...
server.tool(
	...
	'Update multiple accounts at once (1-100). Can set owner, tags, and custom fields in bulk.',
	{
		...
		tags: z
			.array(z.string())
			.optional()
			.describe('Tags to update on the accounts'),
		tags_apply_mode: z
			.enum(['append_only', 'remove_only', 'replace'])
			.optional()
			.describe(
				'How to apply tags: append_only, remove_only, or replace (default: replace)',
			),
		custom_fields: z
			.array(
				z.object({
...
server.tool(
	...
	'List issues within a time range (max 30 days). Returns compact table. Use pylon_get_issue for details.',
	{
		...
		limit: z
			...
			.optional()
			.describe(
				`Number of issues to return (1-100, default ${DEFAULT_ISSUE_LIMIT})`,
			),
		cursor: z.string().optional().describe('Pagination cursor for next page'),
	},
...
server.tool(
	...
	'Update an existing issue',
	{
		...
		requester_id: z
			.string()
			.optional()
			.describe('ID of the requester this issue is on behalf of'),
		type: z
			.enum(['conversation', 'ticket'])
			.optional()
			.describe(
				'Set to "ticket" to upgrade a conversation to a support ticket (cannot be downgraded)',
			),
		custom_fields: z
			.array(
				z.object({
	...
	async ({ id, limit, cursor }) => {
		...
		return {
			content: [
				{
					type: 'text',
					text: JSON.stringify(result.data, null, 2) + pagination,
				},
			],
		};
	},
...
server.tool(
	...
	'Link or unlink an external issue (e.g. Linear ticket) to a Pylon issue.',
	{
		issue_id: z.string().describe('The Pylon issue ID'),
		external_issue_id: z
			.string()
			.describe('The external issue identifier (e.g. "LIN-1234")'),
		source: z.string().describe('The source system (e.g. "linear")'),
		operation: z
			.enum(['link', 'unlink'])
			.optional()
			.describe(
				'Operation to perform (default: "link"). Use "unlink" to remove.',
			),
	},
	async ({ issue_id, external_issue_id, source, operation }) => {
		const result = await client.linkExternalIssue(issue_id, {
			external_issue_id,
			source,
			operation,
		});
...
server.tool(
	...
	'Get all internal threads on an issue',
	{
		...
		limit: z
			...
			.optional()
			.describe(
				`Number of threads to return (1-${MAX_LIST_LIMIT}, default ${DEFAULT_LIST_LIMIT})`,
			),
		cursor: z.string().optional().describe('Pagination cursor for next page'),
	},
	async ({ id, limit, cursor }) => {
...
server.tool(
	...
	async ({ id, name }) => {
		const result = await client.createIssueThread(
			id,
			name ? { name } : undefined,
		);
		return {
...
server.tool(
	...
	'Get all messages on an issue',
	{
		...
		limit: z
			...
			.optional()
			.describe(
				`Number of messages to return (1-${MAX_LIST_LIMIT}, default ${DEFAULT_LIST_LIMIT})`,
			),
		cursor: z.string().optional().describe('Pagination cursor for next page'),
	},
	async ({ id, limit, cursor }) => {
...
server.tool(
	...
	'Send a customer-facing reply on an issue, visible to the requester',
	{
		...
		contact_id: z
			...
			.optional()
			.describe(
				'Contact ID to post the message as. Only one of user_id or contact_id can be provided',
			),
		user_id: z
			.string()
			.optional()
			.describe(
				'User ID to post the message as. If not provided, the API token user will be used',
			),
		attachment_urls: z
			.array(z.string())
			.optional()
...
server.tool(
	...
	'Post an internal note on an issue thread. Not visible to the requester',
	{
		...
		thread_id: z
			...
			.optional()
			.describe(
				'The thread ID to post the note to (use the id field from GET /issues/{id}/threads). Either this or message_id must be provided',
			),
		message_id: z
			.string()
			.optional()
			.describe(
				'The message ID to reply to (must be an internal note). Either this or thread_id must be provided',
			),
		user_id: z
			.string()
			.optional()
			.describe(
				'User ID to post the note as. If not provided, the API token user will be used',
			),
		attachment_urls: z
			.array(z.string())
			.optional()
...
server.tool(
	'pylon_list_users',
	'List all users (team members) in the Pylon organization.',
	{
		limit: z
			.number()
			.min(1)
...
server.tool(
	...
	async ({ limit, cursor }) => {
		...
		const rows = result.data.map(
			(kb) =>
				`| ${kb.id} | ${kb.title} | ${kb.slug} | ${kb.default_language} |`,
		);
		const table = [
...
server.tool(
	...
	async ({ knowledge_base_id, name, slug }) => {
		const result = await client.createKbCollection(knowledge_base_id, {
			name,
			slug,
		});
		return {
...
server.tool(
	...
	async ({ knowledge_base_id, collection_id }) => {
		const result = await client.deleteKbCollection(
			knowledge_base_id,
			collection_id,
		);
		return {
...
server.tool(
	...
	'List all articles in a knowledge base.',
	{
		...
		language: z
			.string()
			.optional()
			.describe('Language code (defaults to KB default language)'),
	},
	async ({ knowledge_base_id, limit, cursor, language }) => {
		const result = await client.listKbArticles(knowledge_base_id, {
			limit,
			cursor,
			language,
		});
		const rows = result.data.map(
			(a) =>
				`| ${a.id} | ${truncate(a.title, MAX_TITLE_LENGTH)} | ${a.slug} | ${
...
server.tool(
	...
	'Create an article in a knowledge base.',
	{
		...
		collection_id: z
			.string()
			.optional()
			.describe('Collection to place the article in'),
		slug: z.string().optional().describe('URL-friendly identifier'),
		is_published: z
			.boolean()
			.optional()
		...
		is_unlisted: z
			.boolean()
			.optional()
			.describe('Whether the article is unlisted (default: false)'),
	},
...
server.tool(
	...
	async ({ knowledge_base_id, article_id, language }) => {
		const result = await client.getKbArticle(
			knowledge_base_id,
			article_id,
			language,
		);
...
server.tool(
	...
	async ({ knowledge_base_id, article_id, ...data }) => {
		const result = await client.updateKbArticle(
			knowledge_base_id,
			article_id,
			data,
		);
...
server.tool(
	...
	async ({ knowledge_base_id, from_path, to_path }) => {
		const result = await client.createKbRouteRedirect(knowledge_base_id, {
			from_path,
			to_path,
		});
		return {
